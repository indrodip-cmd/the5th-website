'use client'
/* Step 2 — Deep Questions (India, embedded Typeform, mandatory). The time is
   already picked; on submit we advance to the Confirmation (thank-you) page. */
    ...
    const mount = (): boolean => {
      ...
      tf.createWidget(formId, {
        ...
        onSubmit: () => {
          ...
          setTimeout(() => { router.push(`/10k-roadmap/india/thank-you${email ? `?email=${encodeURIComponent(email)}` : ''}`) }, 700)
        },
      })
      return true
    }
