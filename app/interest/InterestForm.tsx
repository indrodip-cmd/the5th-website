...
import {
  ...
  PRIMARY_GOALS, HELP_NEEDED, REVENUE_STAGES, PRICING_COUNTRIES, countryByCode,
} from './config'
import {
...
type StepId =
  | 'country' | 'business_type' | 'ai_business_type' | 'niche' | 'business_stage'
  | 'monthly_revenue' | 'primary_goal' | 'help_needed' | 'contact'

interface Answers {
  country: string // 'US' | 'IN' — chosen up front; drives price + dial code
...
const EMPTY: Answers = {
  country: '', business_type: '', ai_business_type: '', niche: '', niche_other: '',
...
export default function InterestForm() {
  ...
  const steps = useMemo<StepId[]>(() => {
    const s: StepId[] = ['country', 'business_type']
  ...
  async function submit() {
    ...
    setSubmitting(true)
    setFormError(null)
    const c = countryByCode(a.country)
    const dial = c?.dial || ''
    const phone = a.phone.trim().startsWith('+') ? a.phone.trim() : `${dial} ${a.phone.trim()}`.trim()
    const countryName = c?.name || a.country
    const priceShown = c ? `${c.pricePrefix} ${c.priceMain}`.trim() : null
    ...
    try {
      const res = await fetch('/api/interest', {
        ...
        body: JSON.stringify({
          ...
          country: countryName, country_code: a.country, price_shown: priceShown,
      ...
      track('interest_form_submitted', {
        country_code: a.country, business_type: a.business_type, business_stage: a.business_stage,
      ...
      <div style={{ width: '100%', maxWidth: 600, margin: '0 auto', padding: '26px 20px 40px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1 }}>
          <AnimatePresence mode="wait" initial={false}>
            ...
            >
              {stepId === 'country' && (
                <CountryStep value={a.country} onSelect={(v) => set({ country: v })} />
              )}
    ...
    <div role="radiogroup" aria-label="Where are you based?">
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        {PRICING_COUNTRIES.map((c) => {
          ...
          return (
            <button
              key={c.code} type="button" role="radio" aria-checked={on}
              onClick={() => onSelect(c.code)}
              className={`io-card${on ? ' io-card--on' : ''}`}
            >
              <span aria-hidden style={{ fontSize: 24, lineHeight: 1 }}>{c.flag}</span>
              <span className="io-card-text" style={{ fontWeight: 600, color: C.ink }}>{c.name}</span>
              <span aria-hidden style={{
                width: 22, height: 22, borderRadius: 999, flex: '0 0 auto',
                border: `2px solid ${on ? C.gold : C.line}`, background: on ? C.gold : 'transparent',
                display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
              }}>
                {on && (
                  <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                    <path d="M2.5 6.2 5 8.6l4.5-5" stroke={C.plumDeep} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                )}
              </span>
            </button>
    ...
    <div>
      ...
      {country && (
        <div style={{ marginBottom: 18 }}>
          <span style={{ display: 'block', fontSize: 12.5, fontWeight: 700, color: C.ink, marginBottom: 7, fontFamily: SANS }}>Country</span>
          <div style={{
            display: 'flex', alignItems: 'center', gap: 8, padding: '13px 15px',
            border: `1px solid ${C.line}`, borderRadius: 12, background: C.creamAlt,
      ...
      <Field id="phone" label="Phone / WhatsApp" type="tel" inputMode="tel" value={a.phone}
        ...
        placeholder="Mobile number" prefix={dial} />
