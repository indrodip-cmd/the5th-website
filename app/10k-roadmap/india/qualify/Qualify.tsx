'use client'
/* Qualification (India), 5 questions, one at a time. A `reject` option routes to
   /10k-roadmap/india/not-a-fit. On success the applicant enters the booking
   wizard at Step 1 (Pick a Time). The qualification answers ride in
   sessionStorage and are saved to the lead when the booking is recorded.
   Reuses the shared UI kit + tracking; only the copy (../config) is India's. */
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: 'clamp(30px,6vw,64px) 22px 80px' }}>
        <QuestionFlow
          ...
          onReject={(reason, answers) => {
            ...
            const bucket = q?.id === 'business_type' ? 'no_business' : q?.id === 'income' ? 'income' : q?.id === 'readiness' ? 'free_advice' : 'default'
            track('qualification_completed'); track('qualification_rejected', { reason: bucket })
            router.push(`/10k-roadmap/india/not-a-fit?r=${bucket}`)
          }}
          onComplete={(answers) => {
            saveQualAnswers(answers)
            track('qualification_completed'); track('qualification_accepted')
            router.push('/10k-roadmap/india/schedule')
          }}
        />
