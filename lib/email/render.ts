...
export const BLOCK_DEFS: Array<{ type: string; label: string; icon: string; group: string; defaults: Row }> = [
  ...
  { type: 'video', label: 'Video thumbnail', icon: '▶', group: 'Premium', defaults: { src: '', url: 'https://the5th.co', alt: 'Watch' } },
  { type: 'columns3', label: 'Three columns', icon: '☰', group: 'Layout', defaults: { c1: 'Column one', c2: 'Column two', c3: 'Column three' } },
  { type: 'badge', label: 'Badge', icon: '●', group: 'Premium', defaults: { text: 'New', align: 'center' } },
  { type: 'progress', label: 'Progress bar', icon: '▰', group: 'Premium', defaults: { label: 'Your progress', percent: 60 } },
  { type: 'countdown', label: 'Deadline', icon: '⏰', group: 'Premium', defaults: { label: 'Offer ends', date: 'Friday at midnight' } },
  { type: 'faq', label: 'FAQ', icon: '?', group: 'Premium', defaults: { items: 'How long is the program?|Five months of coaching + implementation.\nIs there a guarantee?|Yes — 100% money-back if requirements are met.' } },
  { type: 'appointment', label: 'Appointment card', icon: '📅', group: 'Premium', defaults: { title: 'Your strategy call', date: '{{meeting_date}}', time: '{{meeting_time}}', buttonText: 'Add to calendar', buttonUrl: '{{booking_link}}' } },
  { type: 'ai_recommendation', label: 'AI recommendation', icon: '✦', group: 'Premium', defaults: { title: 'Recommended for you', text: 'Based on your goals, {{recommended_product}} is your best next step.', buttonText: 'See why', buttonUrl: 'https://the5th.co' } },
...
function renderBlock(b: Block, brand: Brand, vars?: Row): string {
  ...
  switch (b.type) {
    ...
    case 'video': { const thumb = p.src ? `<img src="${mt(p.src, vars)}" alt="${mt(p.alt, vars)}" width="100%" style="display:block;border-radius:${brand.radius}px;" />` : `<div style="height:200px;background:linear-gradient(135deg,${brand.primary_color},${brand.secondary_color});border-radius:${brand.radius}px;"></div>`; return `<tr><td style="padding:10px 32px;"><a href="${mt(p.url, vars)}" style="display:block;position:relative;text-decoration:none;">${thumb}<span style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:56px;height:56px;background:rgba(0,0,0,.55);border-radius:50%;color:#fff;font-size:22px;line-height:56px;text-align:center;">▶</span></a></td></tr>` }
    case 'columns3': return `<tr><td style="padding:8px 22px;"><table role="presentation" width="100%"><tr>${['c1', 'c2', 'c3'].map((c) => `<td width="33%" style="padding:8px;font-size:13.5px;line-height:1.55;color:${brand.text_color};vertical-align:top;">${mt(p[c], vars)}</td>`).join('')}</tr></table></td></tr>`
    case 'badge': return `<tr><td align="${p.align || 'center'}" style="padding:8px 32px;"><span style="display:inline-block;background:${brand.accent_color};color:#1a1206;font-size:11px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;padding:5px 12px;border-radius:999px;">${mt(p.text, vars)}</span></td></tr>`
    case 'progress': { const pct = Math.max(0, Math.min(100, Number(p.percent) || 0)); return `<tr><td style="padding:10px 32px;">${p.label ? `<div style="font-size:13px;color:${brand.text_color};margin-bottom:6px;">${mt(p.label, vars)} · ${pct}%</div>` : ''}<div style="background:#ece8f0;border-radius:999px;height:10px;"><div style="width:${pct}%;background:${brand.accent_color};height:10px;border-radius:999px;"></div></div></td></tr>` }
    case 'countdown': return `<tr><td align="center" style="padding:12px 32px;"><div style="display:inline-block;border:1px solid ${brand.accent_color};border-radius:${brand.radius}px;padding:12px 22px;"><div style="font-size:12px;letter-spacing:.06em;text-transform:uppercase;color:${brand.accent_color};font-weight:700;">⏰ ${mt(p.label, vars)}</div><div style="font-size:18px;font-weight:800;color:${brand.primary_color};margin-top:4px;">${mt(p.date, vars)}</div></div></td></tr>`
    case 'faq': return `<tr><td style="padding:10px 32px;">${String(p.items || '').split('\n').filter(Boolean).map((row) => { const [q, a] = row.split('|'); return `<div style="margin-bottom:12px;"><div style="font-size:14.5px;font-weight:700;color:${brand.primary_color};">${mt(q, vars)}</div><div style="font-size:14px;color:${brand.text_color};line-height:1.55;margin-top:3px;">${mt(a || '', vars)}</div></div>` }).join('')}</td></tr>`
    case 'appointment': return `<tr><td style="padding:12px 24px;"><table role="presentation" width="100%" style="background:#faf8fc;border:1px solid #e6e2ea;border-radius:${brand.radius + 4}px;"><tr><td style="padding:20px 24px;"><div style="font-size:12px;letter-spacing:.06em;text-transform:uppercase;color:${brand.accent_color};font-weight:700;">📅 Appointment</div><div style="font-size:17px;font-weight:800;color:${brand.primary_color};margin:4px 0;">${mt(p.title, vars)}</div><div style="font-size:14px;color:${brand.text_color};">${mt(p.date, vars)} · ${mt(p.time, vars)}</div><a href="${mt(p.buttonUrl, vars)}" style="display:inline-block;margin-top:12px;background:${brand.accent_color};color:#1a1206;font-weight:700;font-size:14px;text-decoration:none;padding:10px 20px;border-radius:${brand.radius}px;">${mt(p.buttonText, vars)}</a></td></tr></table></td></tr>`
    case 'ai_recommendation': return `<tr><td style="padding:12px 24px;"><table role="presentation" width="100%" style="background:linear-gradient(135deg,#2A1830,#160D1A);border-radius:${brand.radius + 4}px;"><tr><td style="padding:22px 24px;"><div style="font-size:12px;letter-spacing:.06em;text-transform:uppercase;color:${brand.accent_color};font-weight:700;">✦ ${mt(p.title, vars)}</div><div style="font-size:15px;color:#fff;line-height:1.55;margin:8px 0 14px;">${mt(p.text, vars)}</div><a href="${mt(p.buttonUrl, vars)}" style="display:inline-block;background:${brand.accent_color};color:#1a1206;font-weight:700;font-size:14px;text-decoration:none;padding:10px 22px;border-radius:${brand.radius}px;">${mt(p.buttonText, vars)}</a></td></tr></table></td></tr>`
